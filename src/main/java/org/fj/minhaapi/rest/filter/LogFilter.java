...
public class LogFilter implements ContainerRequestFilter, ContainerResponseFilter {
	...
	public void filter(ContainerRequestContext request) {
		logger.info("[REQUEST] " + properties(request) + ";" + request.getMethod() + ";" + headers(request) + request.getUriInfo().getPath() + ";" + " "
				             + LocalDateTime.now().format(DateTimeFormatter.ISO_DATE_TIME));
	}

	@Override
	public void filter(ContainerRequestContext request, ContainerResponseContext response) {
		logger.info("[RESPONSE]" + properties(request) + ";" + ";" + request.getMethod() + ";" + request.getUriInfo().getPath() + ";" + headers(request) + response.getStatusInfo()
				             + ";" + " " + LocalDateTime.now().format(DateTimeFormatter.ISO_DATE_TIME));
	}
